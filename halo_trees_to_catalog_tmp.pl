...
sub find_mass_years_ago {
    ...
    # # Enia added some code
    # { print "$halos{$guess->{id}}\n" }
...
sub calc_accretion_rates {
	...
	$h->{acc_dyn} = ($h->{orig_mvir}-$mtdyn) / $tdyn; # ENIA 
	...
	if ($h->{scale} == 1.1){
		$tdynenia = 2*$tdyn;
		print "Halo ID $h->{id}\n";
		print "halo mass now $h->{orig_mvir}\n";
		print "halo mass then $m2tdyn\n";
		print "dynamical time $tdynenia\n";
		print "accretion rate $h->{acc_2dyn} \n";
		print "scale factor now $h->{scale}\n";
		print "scale factor then $h2dyn->{scale}\n";
		print "ID now $h->{id}\n";
		print "ID then $h2dyn->{id}\n\n";
	}
...
sub print {
    ...
    $h->{acc_inst}, $h->{acc_100}, $h->{acc_dyn}, $h->{acc_2dyn}, $h->{acc_mpeak}, $h->{dlvmax_inst}, $h->{dlvmax_dyn}, $h->{mpeak_scale}, $h->{acc_scale}, $h->{first_acc}{scale}, $h->{first_acc}{orig_mvir}, $h->{first_acc}{vmax}, $h->{vmpeak}, $h->{tidal_av}, $h->{lvmax_tdyn}, 
    $tt_merger, $merger_mmp, $times{$h->{scale}});
}
